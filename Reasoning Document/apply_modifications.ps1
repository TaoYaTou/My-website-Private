$path = 'D:\Application\AllToolsSet\Herb\Smart-Chinese-Herbal-Medicine-Recognition-App\index.html'
$content = [System.IO.File]::ReadAllText($path, [System.Text.Encoding]::UTF8)
$origLen = $content.Length

function Replace-Block($old, $new) {
    if (-not $content.Contains($old)) {
        Write-Host "NOT FOUND: $($old.Substring(0,80).Replace("`n",'\n'))"
    } else {
        $script:content = $content.Replace($old, $new)
        Write-Host "REPLACED: $($old.Substring(0,60).Replace("`n",'\n'))"
    }
}
...
'@
Replace-Block $oldRoot $newRoot

# 2. Add extra CSS
$oldCss = @'
        .lock-timer { color: #ef4444; font-size: 0.9rem; margin-top: 0.5rem; }
...
$newCss = @'
        ...
        /* Toast */
'@
Replace-Block $oldCss $newCss

# 3. Reorder project panel
$oldProject = @'
            <div class="project-wide-panel reveal">
...
'@
Replace-Block $oldProject $newProject

# 4. Reveal classes (applied separately after color replacement)

# 5. Admin section HTML
$oldAdminHtml = @'
    <!-- Admin -->
    <section id="admin">
        <div class="container">
...
'@
Replace-Block $oldAdminHtml $newAdminHtml
...
'@
Replace-Block $oldProjectClick $newProjectClick
...
# 9. selectFile and file descriptions
$oldSelect = @'
        // ===================== Detail panel =====================
...
'@
Replace-Block $oldSelect $newSelect

# 10. Folder geometry thinner
$oldFolder = @'
            function createFolderGeometry() {
...
$newFolder = @'
            ...
            function createGearGeometry() {
'@
Replace-Block $oldFolder $newFolder

# 11. Radial node placement
$oldCreateNode = @'
            function createNode(node, parentPath, parentMesh, depth) {
...
'@
$newCreateNode = @'
            function createNode(node, parentPath, parentMesh, depth, childIndex, siblingCount) {
                const path = parentPath ? parentPath + '/' + node.name : node.name;
                let x, y, z;
                if (!parentMesh) {
                    x = 0; y = 0; z = 0;
...
'@
Replace-Block $oldCreateNode $newCreateNode
...
'@
Replace-Block $oldAdminJs $newAdminJs

# 13. Global color replacements
$colors = @(
    @('#3b82f6','#007aff'),
    @('0x3b82f6','0x007aff'),
    @('#8b5cf6','#5e5ce6'),
...
foreach ($pair in $colors) {
    ...
    Write-Host "GLOBAL: $($pair[0]) -> $($pair[1])"
}

[System.IO.File]::WriteAllText($path, $content, [System.Text.Encoding]::UTF8)
Write-Host "Done. Original $origLen -> New $($content.Length)"
