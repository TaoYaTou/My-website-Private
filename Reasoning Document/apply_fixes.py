...
def find_function_end(code, start):
    """Find the index of the closing brace that matches the first '{' after start.
    Handles strings, template literals and ${} expressions."""
    stack = [{'name': 'code', 'depth': 1}]
    i = start
    while i < len(code):
        state = stack[-1]
        ch = code[i]
        if state['name'] == 'code' or state['name'] == 'template_expr':
            if ch == '{' and state['name'] == 'code':
                state['depth'] += 1
            elif ch == '}' and state['name'] == 'code':
                state['depth'] -= 1
                if state['depth'] == 0:
                    stack.pop()
                    if not stack:
                        return i
            elif ch == '{' and state['name'] == 'template_expr':
                state['depth'] += 1
            elif ch == '}' and state['name'] == 'template_expr':
                state['depth'] -= 1
                if state['depth'] == 0:
                    stack.pop()  # back to backtick
            elif ch == "'":
                stack.append({'name': 'single'})
            elif ch == '"':
                stack.append({'name': 'double'})
            elif ch == '`':
                stack.append({'name': 'backtick'})
        elif state['name'] == 'single':
            if ch == '\\':
                i += 1
            elif ch == "'":
                stack.pop()
        elif state['name'] == 'double':
            if ch == '\\':
                i += 1
            elif ch == '"':
                stack.pop()
        elif state['name'] == 'backtick':
            if ch == '\\':
                i += 1
            elif ch == '`':
                stack.pop()
            elif code.startswith('${', i):
                stack.append({'name': 'template_expr', 'depth': 1})
                i += 1
        i += 1
    return -1

def replace_function(code, name, new_text, async_fn=False):
    """Replace a top-level JS function declaration (possibly async)."""
    prefix = r'async\s+' if async_fn else r''
    pattern = prefix + r'function\s+' + re.escape(name) + r'\s*\([^)]*\)\s*\{'
    m = re.search(pattern, code)
    if not m:
        raise ValueError(f'Function {name} not found')
    start = m.start()
    brace_start = m.end() - 1  # position of '{'
    end = find_function_end(code, brace_start)
    if end < 0:
        raise ValueError(f'Could not find end of function {name}')
    return code[:start] + new_text + code[end+1:]

def add_after_marker(code, marker, text):
    if marker not in code:
        raise ValueError(f'Marker not found: {marker}')
    idx = code.find(marker) + len(marker)
    return code[:idx] + text + code[idx:]
...
# --- CSS & HTML: user menu, scrollable list classes, participants inputs, mail modal ---
...
code = code.replace(
    '<ul class="profile-list" id="existingFeaturesList" style="margin-top:1rem;">',
    '<ul class="profile-list scrollable-list" id="existingFeaturesList" style="margin-top:1rem;">'
)
code = code.replace(
    '<ul class="project-list" id="plannedProjectsList">',
    '<ul class="project-list scrollable-list" id="plannedProjectsList">'
)
code = code.replace(
    '<ul class="project-list" id="ongoingProjectsList">',
    '<ul class="project-list scrollable-list" id="ongoingProjectsList">'
)
code = code.replace(
    '<ul class="project-list" id="completedProjectsList">',
    '<ul class="project-list scrollable-list" id="completedProjectsList">'
)

# Participants input in feature apply form
code = code.replace(
...
# Participants input in project apply form
code = code.replace(
    '''                        <div class="form-group">
                            <label>联系方式 <span class="required">*</span></label>
                            <input type="text" name="contact" required placeholder="手机号/邮箱/微信号">
...
# Participants input in profile submit form
code = code.replace(
    '''                    <div class="form-group">
                        <label>展示文件提交</label>
...
'''
code = add_after_marker(code, "function setApplications(apps) { localStorage.setItem('applications', JSON.stringify(apps)); }", notification_helpers)

# --- Replace JS functions ---

code = replace_function(code, 'saveApplication', '''function saveApplication(type, name, category, description, contact, host = '访客', extraParticipants = []) {
...
code = replace_function(code, 'renderProjectLists', '''function renderProjectLists() {
            const apps = getApplications();
...
code = replace_function(code, 'renderFeatureList', '''function renderFeatureList() {
            const apps = getApplications();
...
code = replace_function(code, 'showProjectDetail', '''function showProjectDetail(projectId) {
            const apps = getApplications();
...
code = replace_function(code, 'loadProject', '''function loadProject(projectId) {
...
code = replace_function(code, 'renderProfile', '''function renderProfile() {
            const session = getCurrentSession();
            if (!session) return;
...
code = replace_function(code, 'renderProfileViolations', '''function renderProfileViolations(user) {
            const apps = getApplications();
...
code = replace_function(code, 'finalizeLogin', '''async function finalizeLogin(user) {
...
code = replace_function(code, 'renderNavUser', '''function renderNavUser() {
            const session = getCurrentSession();
            if (!session) {
...
code = replace_function(code, 'handleApproveRegistration', '''async function handleApproveRegistration(id, approve) {
            const regs = getRegistrations();
...
code = replace_function(code, 'renderRegistrations', '''function renderRegistrations() {
...
code = replace_function(code, 'handleDismissAdmin', '''async function handleDismissAdmin(e) {
...
code = replace_function(code, 'renderUserList', '''function renderUserList() {
...
code = replace_function(code, 'renderApplicationsManagement', '''function renderApplicationsManagement(role) {
...
code = replace_function(code, 'renderViolations', '''function renderViolations() {
...
code = replace_function(code, 'handleViolationReview', '''async function handleViolationReview(id, confirm) {
            const allApps = getApplications();
            const app = allApps.find(a => a.id === id);
...
code = replace_function(code, 'handleShowcaseReview', '''async function handleShowcaseReview(id, approve) {
            const allApps = getApplications();
            const app = allApps.find(a => a.id === id);
            if (!app) return;
            const session = getCurrentSession();
...
# Add handleDeleteApplication after handleShowcaseReview
code = add_after_marker(code, "renderProjectList();\n        }", '''\n\n        async function handleDeleteApplication(id) {
            const apps = getApplications();
...
code = replace_function(code, 'renderLogs', '''function renderLogs() {
...
# --- Update form handlers to pass participants ---
...
# Update profile submit handler to merge participants
old_profile_submit = '''            if (existing && existing.status !== '已退回') {
                if (file) {
...
new_profile_submit = '''            const newParticipants = (fd.get('participants') || '').split(/[,，]/).map(s => s.trim()).filter(Boolean);
...
if old_profile_submit not in code:
    raise ValueError('Profile submit block not found')
code = code.replace(old_profile_submit, new_profile_submit)

# --- Add event listeners for mail modal ---
code = code.replace(
